...
def now_dt() -> pd.Timestamp:
    """Current app time as naive Europe/Rome timestamp.

    CSV datetimes such as lock_timestamp are stored without timezone and are
    intended to represent Rome time. Streamlit Cloud servers may run in UTC,
    so always convert current time to Europe/Rome before comparing/saving.
    """
...
def zero_zero_prediction(row) -> bool:
    return to_int_or_none(row.get("pred_home_score")) == 0 and to_int_or_none(row.get("pred_away_score")) == 0


def zero_zero_real_result(row) -> bool:
    return to_int_or_none(row.get("home_score")) == 0 and to_int_or_none(row.get("away_score")) == 0


def prediction_has_own_goal(row) -> bool:
    return bool_value(row.get(PRED_OG_1)) or bool_value(row.get(PRED_OG_2))


def real_result_has_own_goal(row) -> bool:
    return bool_value(row.get(REAL_OG_1)) or bool_value(row.get(REAL_OG_2))


def player_team_from_roster(md_players: pd.DataFrame, player_name: str) -> str:
    """Return the national team for a player, when md_players.csv allows it."""
    player_name = clean_value(player_name)
    if player_name == "" or md_players is None or md_players.empty:
        return ""
    required_cols = {"team", "player_name"}
    if not required_cols.issubset(md_players.columns):
        return ""
    matches_player = md_players[
        md_players["player_name"].fillna("").astype(str).str.strip().str.lower() == player_name.lower()
    ]
    if matches_player.empty:
        return ""
    return clean_value(matches_player.iloc[0].get("team", ""))


def prediction_consistency_errors(row, home_team: str, away_team: str, md_players: pd.DataFrame | None = None) -> list[str]:
    """Validate that scorer/own-goal choices are coherent with Gol Casa/Gol Trasferta.

    Semantics used by the app:
    - blank scorer + no own-goal flag = intentional "nessun marcatore"; this is coherent only with 0-0.
    - pred_autogol_squadra_1 means squadra 1/home team scored an own goal, so it gives a goal to squadra 2/away team: Gol Trasferta must be > 0.
    - pred_autogol_squadra_2 means squadra 2/away team scored an own goal, so it gives a goal to squadra 1/home team: Gol Casa must be > 0.
    - when md_players.csv is available and the scorer is found there, the scorer's team must have at least one predicted goal.
    """
    errors: list[str] = []
    pred_home = to_int_or_none(row.get("pred_home_score"))
    pred_away = to_int_or_none(row.get("pred_away_score"))
    ...
    has_any_prediction_detail = (
        clean_value(row.get("pred_home_score", "")) != ""
        or clean_value(row.get("pred_away_score", "")) != ""
        or pred_scorer != ""
        or og1
        or og2
    )

    # Entirely blank row: still editable later and not considered an inconsistent pronostic.
    if not has_any_prediction_detail:
        return errors

    if pred_home is None or pred_away is None:
        errors.append("devi indicare sia Gol Casa sia Gol Trasferta prima di inserire marcatore/autogol")
        return errors

    if pred_home < 0 or pred_away < 0:
        errors.append("Gol Casa e Gol Trasferta non possono essere negativi")
        return errors

    total_goals = pred_home + pred_away

    if pred_scorer != "" and (og1 or og2):
        errors.append("marcatore e autogol sono alternativi: scegli solo una delle due opzioni")

    if og1 and og2:
        errors.append("puoi flaggare al massimo un autogol nello stesso pronostico")

    if total_goals == 0:
        if pred_scorer != "" or og1 or og2:
            errors.append("con risultato esatto 0-0 il marcatore deve restare blank e non puoi flaggare autogol")
        return errors

    if pred_scorer == "" and not og1 and not og2:
        errors.append("se pronostichi almeno un gol, devi indicare un marcatore oppure un autogol")

    if og1 and pred_away == 0:
        errors.append("Autogol squadra 1 è coerente solo se Gol Trasferta è maggiore di 0")

    if og2 and pred_home == 0:
        errors.append("Autogol squadra 2 è coerente solo se Gol Casa è maggiore di 0")

    scorer_team = player_team_from_roster(md_players, pred_scorer)
    home_team = clean_value(home_team)
    away_team = clean_value(away_team)

    if pred_scorer != "" and scorer_team != "":
        if scorer_team == home_team and pred_home == 0:
            errors.append(f"{pred_scorer} appartiene a {home_team}: Gol Casa deve essere maggiore di 0")
        elif scorer_team == away_team and pred_away == 0:
            errors.append(f"{pred_scorer} appartiene a {away_team}: Gol Trasferta deve essere maggiore di 0")
        elif scorer_team not in [home_team, away_team]:
            errors.append(f"{pred_scorer} non appartiene alle squadre della partita selezionata")
...
def scorer_correct(row) -> bool:
    ...
    # Regola di mutua esclusione: il bonus marcatore normale non si valuta
    # quando il pronostico è stato espresso come autogol.
    if prediction_has_own_goal(row):
        return False

    # Blank intenzionale: se il partecipante non indica un marcatore e non
    # flagga autogol, il pronostico vale come "nessun marcatore". Il bonus
    # marcatore si assegna solo quando anche la partita reale finisce 0-0,
    # senza marcatori reali e senza autogol reali.
    if pred_scorer == "":
        return (
            zero_zero_prediction(row)
            and zero_zero_real_result(row)
            and real_scorers == ""
            and not real_result_has_own_goal(row)
        )
...
def own_goal_correct(row) -> bool:
    ...
    # Regola di mutua esclusione anche in calcolo: se un vecchio CSV contiene
    # sia marcatore sia flag autogol, non assegna il bonus autogol.
    if pred_scorer != "":
        return False
...
def create_backup() -> Path:
    ensure_data_dir()
    backup_path = BACKUP_DIR / f"backup_{now_dt().strftime('%Y%m%d_%H%M%S')}"
                    ...
                    with c1:
                        if st.button("✅ Applica marcatore al pronostico", key=f"apply_pred_scorer_{selected_partecipant}_{selected_match_id}"):
                            ...
                            if selected_player == "-- Nessun marcatore --":
                                # Blank intenzionale: nessun marcatore e nessun autogol.
                                # Darà bonus marcatore solo in caso di 0-0 reale.
        ...
        if not unlocked_view.empty:
            edited = st.data_editor(
                ...
                column_config={
                    ...
                    "pred_scorer": st.column_config.TextColumn("Marcatore", help="Blank senza autogol = nessun marcatore intenzionale; dà bonus solo se la partita finisce 0-0. Non usare OG/autogol: usa i flag dedicati."),
                    PRED_OG_1: st.column_config.CheckboxColumn("Autogol squadra 1", help="Autogol commesso dalla squadra 1/casa: coerente solo se Gol Trasferta > 0."),
                    PRED_OG_2: st.column_config.CheckboxColumn("Autogol squadra 2", help="Autogol commesso dalla squadra 2/trasferta: coerente solo se Gol Casa > 0."),
            ...
            if st.button("💾 Salva pronostici"):
                ...
                for _, row in edited.iterrows():
                    ...
                    match_row = matches[matches["match_id"].astype(int) == match_id]
                    home_team = clean_value(match_row.iloc[0].get("home_team", "")) if not match_row.empty else ""
                    away_team = clean_value(match_row.iloc[0].get("away_team", "")) if not match_row.empty else ""
                    ...
                    validation_row = {
                        ...
                        "pred_scorer": pred_scorer,
                        PRED_OG_1: og1,
                        PRED_OG_2: og2,
                    }
                    row_errors = prediction_consistency_errors(validation_row, home_team, away_team, md_players)
                    if row_errors:
                        validation_errors.append(
                            f"Partita {match_id} ({home_team} - {away_team}): " + "; ".join(row_errors)
                        )
                        continue
                ...
                if validation_errors:
                    st.error("Correggi i pronostici non coerenti con Gol Casa/Gol Trasferta prima di salvare:")
