        ...
        if not unlocked_view.empty:
            ...
            if st.button("💾 Salva pronostici"):
                ...
                for _, row in edited.iterrows():
                    ...
                    # Marcatore e autogol sono mutuamente esclusivi nel pronostico.
                    # Blank + nessun flag autogol è invece una scelta intenzionale:
                    # significa "nessun marcatore" e può prendere bonus sullo 0-0.
                    if pred_scorer != "" and (og1 or og2):
                        conflict_rows.append(match_id)
                        continue
                ...
                if conflict_rows:
                    st.error(
                        "Correggi prima di salvare: in queste partite hai inserito sia un marcatore sia un flag autogol: "
                        + ", ".join(str(x) for x in conflict_rows)
                        + ". Scegli una sola opzione oppure lascia il marcatore blank senza flag per pronosticare nessun marcatore."
                    )
                    st.stop()
